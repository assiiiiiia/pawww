...
export default {
  ...
  markTaskCompleted(event) {
    ...
    const updatedTask = {
      ...this.task,
      completed: isChecked,
      status: isChecked ? "terminé" : this.task.status,
    };

    // Emit the updated task status to the parent component to update the UI
    this.$emit("update-task", updatedTask);

    // Send a request to the backend to update the task status
    axios.put(`http://localhost:3000/api/tasks/${this.task.id}/status`, {
      completed: isChecked
    })
    .then(response => {
      console.log('Task status updated to "terminé":', response.data.message);
    })
    .catch(error => {
      console.error('Error updating task status:', error);
    });
