...
router.post('/tasks-add', isAuthenticated, (req, res) => {
  ...
  const query = `INSERT INTO tasks (task_name, category, due_date, due_time, priority, user_id) VALUES (?, ?, ?, ?, ?, ?)`;
  const values = [
...
router.put('/tasks/:id', isAuthenticated, (req, res) => {
  ...
  db.query(taskOwnershipQuery, [taskId, userId], (err, result) => {
    ...
    if (task_name !== undefined) {
      ...
      values.push(existingTask.task_name); // Keep the existing value if no update
    }

    // If no values are provided for category, use the existing one
    if (category !== undefined) {
      updates.push('category = ?');
      values.push(category);
    } else {
      values.push(existingTask.category); // Keep the existing value if no update
    }

    // Update the date if provided, or keep the existing one
    if (due_date !== undefined) {
      updates.push('due_date = ?');
      values.push(due_date);
    } else {
      values.push(existingTask.due_date); // Keep the existing value if no update
    }

    // Update the time if provided, or keep the existing one
    if (due_time !== undefined) {
      updates.push('due_time = ?');
      values.push(due_time);
    } else {
      values.push(existingTask.due_time); // Keep the existing value if no update
    }

    // If no values are provided for priority, use the existing one
    if (priority !== undefined) {
      updates.push('priority = ?');
      values.push(priority);
    } else {
      values.push(existingTask.priority); // Keep the existing value if no update
    }

    // If no values are provided for status, use the existing one
    if (status !== undefined) {
      updates.push('status = ?');
      values.push(status);
    } else {
      values.push(existingTask.status); // Keep the existing value if no update
    }
    ...
    db.query(updateQuery, values, (err, result) => {
      ...
      // Send back the updated task data, or the original if no update
      const updatedTask = {
        task_name: task_name || existingTask.task_name,
        category: category || existingTask.category,
        due_date: due_date || existingTask.due_date,
        due_time: due_time || existingTask.due_time,
